...
// --- セーブ・ロード機能 ---
const SAVE_KEY = 'shooterGameSave'; // localStorage で使用するキー
...
function getGameState() {
    return {
        ...
        upgrades: JSON.parse(JSON.stringify(UPGRADES)) // UPGRADESオブジェクトをディープコピー
    };
}

/**
 * ゲーム状態をlocalStorageに保存する (セーブ)
 */
function saveGame() {
    const gameState = getGameState();
    try {
        localStorage.setItem(SAVE_KEY, JSON.stringify(gameState));
        console.log('ゲームを保存しました:', gameState);
        // プレイヤーに通知するUI要素があれば、ここで更新できます
    } catch (e) {
        console.error('ゲームの保存に失敗しました:', e);
    }
}
window.saveGame = saveGame; // HTMLから呼び出せるようにグローバルスコープに追加

/**
 * ゲーム状態をlocalStorageからロードする
 * @returns {boolean} ロードに成功したかどうか
 */
function loadGame() {
    try {
        ...
        if (savedState) {
            ...
            playerHealth = gameState.playerHealth || 3;
            ...
            console.log('ゲームをロードしました:', gameState);
...
// ロード機能を追加したので、セーブデータを消す機能も追加しておくと便利です
function clearSave() {
    localStorage.removeItem(SAVE_KEY);
    console.log('セーブデータを削除しました。');
    // セーブデータを消した後、画面をリロードして初期状態に戻します
    window.location.reload(); 
}
window.clearSave = clearSave; // HTMLから呼び出せるようにグローバルスコープに追加
...
// ロードに成功しなかった場合、またはセーブデータがない場合の初期化
if (!isLoaded) {
